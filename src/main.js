...
const createEditTripEventsItem = () => {
  return (
...
for (let i = 0; i < POINT_COUNT; i++) {
  ...
  render (tripDaysItem, createDayInfoTemplate(), `afterbegin`);
  ...
  for (let j = 0; j < POINT_COUNT; j++) {
    ...
    render (tripEventsItem, createTripEventsItem(), `afterbegin`);
    render (tripEventsItem, createEditTripEventsItem(), `afterbegin`);
    tripEventsList.appendChild(tripEventsItem);
  };
  tripDaysItem.appendChild(tripEventsList);
};

// add-new-event-without-destination && add-new-event-without-destination
