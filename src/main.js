...
const createTripLoadingTemplate = () => {
  return (
...
const createFormTripEventTemplate = () => {
  return (
    `<form class="trip-events__item  event  event--edit" action="#" method="post">
      <header class="event__header">
        <div class="event__type-wrapper">
...
const createEventDistinationTemplate = () => {
  return (
...
const createFormTripSearchTemplate = () => {
  return (
...
const createFormEventEditTemplate = () => {
  return (
...
render(tripInfo, createTripCostTemplate(), `afterbegin`);
render(tripInfo, createTripTitleTemplate(), `afterbegin`);
render(tripControls.children[0], createSiteMenuTemplate(), `afterend`);
render(tripControls.children[2], createFilterFormTemplate(), `afterend`);
...
//no-points
render(tripEvents, createFirstPointTemplate(), `beforeend`);

//list-loading
render(tripEvents, createTripLoadingTemplate(), `beforeend`);

//add-first-event
render(tripEvents, createFormEventTemplate(), `beforeend`);

//list
render(tripEvents, createFormSortTemplate(), `beforeend`);
const tripDays = document.createElement("ul");
tripDays.className = "trip-days";
tripEvents.appendChild(tripDays);
for (let i = 0; i < POINT_COUNT; i++) {
  let tripDaysItem = document.createElement("li");
  tripDaysItem.classList.add("trip-days__item", "day");
  render (tripDaysItem, createTripDaysItemTemplate(i + 1), `beforeend`);
  tripDays.appendChild(tripDaysItem);
};

//list-sort
render(tripMain, createFormTripSearchTemplate(), `afterend`);
tripEvents.querySelector(".trip-sort__item--day").innerHTML = "";
for (var i = 0; i < tripDays.childElementCount; i++) {
  tripDays.children[i].querySelector(".day__info").innerHTML = "";
};

// add-new-event-without-destination
render(tripEvents.children[1], createFormTripEventTemplate(), `afterend`);

// add-new-event-with-destination
const tripEventsItem = document.querySelector(".trip-events__item");
const eventDetails = tripEventsItem.querySelector(".event__details");
render(eventDetails, createEventDistinationTemplate(), `beforeend`);

// edit-event
const tripEventItem = tripEvents.querySelector(".trip-events__item");
const eventHeader = tripEventItem.querySelector(".event__header");
const buttonReset = eventHeader.querySelector(".event__reset-btn");
buttonReset.textContent = "Delete";
render (eventHeader, createFormEventEditTemplate(), `beforeend`);
